...
class GutMicrobiomeHealthIndex():
    ...
    def fit(self, health, nonhealth, ThetaF=0, ThetaD=0, n_jobs=1, low_abundance=1e-5, MaxF=2, MaxD=1, step=0.1):
        # Filter
        health[health < low_abundance] = 0
        nonhealth[nonhealth < low_abundance] = 0
        
        # params search
        if (ThetaF == 0) or (ThetaD == 0):
            RangeThetaF = np.arange(1+step, MaxF, step)
            RangeThetaD = np.arange(step, MaxD, step)
            ...
            ThetaSearch = Parallel(n_jobs=n_jobs)(delayed(self.xMM)(health, nonhealth, i[0], i[1]) for i in RangeThetaFD)
